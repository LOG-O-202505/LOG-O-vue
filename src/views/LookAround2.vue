...
import koreaGeoJson from '@/assets/korea.json'; // 한국 행정구역 GeoJSON 파일
...
export default {
    ...
    setup() {
        ...
        const regionsData = ref({
            ...
            // 인천 데이터 추가/수정 (seoul 데이터 수정)
            'seoul': {
                id: 'seoul',
                name: '서울', // 서울/경기에서 서울로 변경
                color: '#ffffff',
            ...
            'Jeonnam': { // 'gwangju'에서 'Jeonnam'으로 변경
                id: 'Jeonnam',
                name: '전라남도',
                color: '#ffffff',
            ...
            'Gyeongbuk': { // 'daegu'에서 'Gyeongbuk'으로 변경
                id: 'Gyeongbuk',
                name: '경상북도',
                color: '#ffffff',
        ...
        const renderMap = () => {
            ...
            const svg = d3.select(container)
                ...
                .attr('style', 'width: 100%; height: 100%; max-width: 100%;');

            // 지도 그룹 요소 생성
            const g = svg.append('g');

            // 지도 투영법 설정 - 메르카토르 투영법 적용
            const projection = d3.geoMercator()
                .center([128, 36]) // 한국 중심 좌표
                .scale(width * 3.5) // 적절한 크기 조정
            ...
            // 지도 데이터 로드 및 그리기
            // GeoJSON 파일로부터 한국 행정구역 그리기
            const regions = g.selectAll('path')
                .data(koreaGeoJson.features)
                .enter()
                .append('path')
                .attr('class', 'region')
                .attr('id', d => d.properties.id) // 각 지역의 ID 설정
                .attr('d', path)
                .attr('fill', '#a3a3a3')
                .attr('stroke', '#ffffff')
            ...
            regions.on('mouseout', function (event, d) {
                ...
                if (activeRegion.value !== d.properties.id) {
                    d3.select(this)
                        .attr('fill', '#a3a3a3')
                        .attr('stroke-width', 0.5);
                }
